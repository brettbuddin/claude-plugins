...
description: >-
  ...
  into an RFD-style document at docs/reports/REPORT_TOPIC.md.
---
...
4. Glob for `docs/plans/*.md`. If none exist, tell the user to run `/plan` first and stop. By default, use the most recently modified plan. If the user requests specific plans, use those. If multiple plans exist and the user did not specify which to use, the agent will disambiguate.
...
- The specific plan file path(s) to synthesize (e.g., `docs/plans/goal-name.md`)
